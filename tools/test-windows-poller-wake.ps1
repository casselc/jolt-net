...
<#
.SYNOPSIS
  Run jolt-net's dependency-free PUBLIC poller and wake suite on native Windows
  (task W4).

.DESCRIPTION
  Invokes the proposal Jolt runtime's Chez Scheme entry point directly against
  this checkout's -M:poller-wake-test alias. Deliberately does NOT go through
  ...
  Unlike the W1, W2, and W3 runners, this suite drives the PUBLIC
  jolt.net/open-poller and the public blocking accept over real Winsock
  sockets. Task W3 left the public Windows poller fail-closed because it had no
  owner-independent wake transport; task W4 supplies one (a connected IPv4
  loopback datagram pair) and this gate is what establishes that the promotion
...
.PARAMETER TimeoutSeconds
  ...
  one also bounds failures occurring before that main starts. The suite runs
  two handle-leak stress loops and several forced interleavings, so this
  default is larger than the W3 one.
#>
param(
...
if (-not (Test-Path $ChezExe)) {
  throw "test-windows-poller-wake.ps1: scheme.exe not found at $ChezExe"
}
if (-not (Test-Path (Join-Path $RuntimePath "host\chez\cli.ss"))) {
  throw "test-windows-poller-wake.ps1: host\chez\cli.ss not found under $RuntimePath"
}
...
try {
  $process = Start-Process `
    -FilePath $ChezExe `
    -ArgumentList @("--script", "host\chez\cli.ss", "-M:poller-wake-test") `
    -NoNewWindow `
    -PassThru
  ...
  if ($process.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    [Console]::Error.WriteLine(
      "test-windows-poller-wake.ps1: timed out after $TimeoutSeconds seconds; terminating PID $($process.Id)"
    )
    try {
      $process.Kill()
      $process.WaitForExit()
      ...
      [Console]::Error.WriteLine(
        "test-windows-poller-wake.ps1: failed to terminate timed-out PID $($process.Id): $($_.Exception.Message)"
      )
    }
    $exitCode = 124
  }
