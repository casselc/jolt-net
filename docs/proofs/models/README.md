...
ownership/completion, readiness-token, short-lease, non-blocking-transition,
accept-terminal-close, wake-pair, and wake-epoch models.
...
accept-terminal-close, wake-pair, wake-epoch, and short-lease families leaves 9
`.smt2` files in the current tree. The formal CI above now checks all nine
...
| `short-lease.smt2` | `unsat`, four `sat` mutants, eight `sat` boundaries | corrected consistency; late admission, escaped syscall, early native close, blocking wait cycle; admitted/completed/rejected leases and incomplete/complete wait graphs |
...
short lease corrected:
  reference-admission-penalty-definition reference-syscall-penalty-definition
  reference-lease-order-penalty-definition
  reference-native-close-penalty-definition
  reference-cycle-definition-penalty-definition
  reference-deadlock-penalty-definition reference-distance-definition
  reference-classifier-definition implementation-admission-rule-definition
  implementation-syscall-rule-definition
  implementation-lease-order-rule-definition
  implementation-native-close-rule-definition
  implementation-cycle-rule-definition
  implementation-deadlock-free-rule-definition
  implementation-classifier-definition violation-definition
  corrected-selector corrected-counterexample-query
...
- `jolt.net.handle/acquire!`, `release!`, and `close!` implement the
  open/admit, drain, and native-close transitions used by the short-lease model.
  A synthetic native-call oracle proves that close rejects a later lease, emits
  no native close before the admitted lease releases, and emits exactly one
  native close afterward.
